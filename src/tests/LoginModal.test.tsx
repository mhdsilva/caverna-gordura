...
describe('LoginModal', () => {
  ...
  it('renders modal and buttons', () => {
    render(
      <LoginModal onClose={onClose} onLogin={onLogin} onContinueWithoutLogin={onContinue} />
    );
    expect(screen.getByText('Bem-vindo à Caverna da Gordura')).toBeInTheDocument();
  ...
  it('is accessible with role dialog and focus trap', () => {
    ...
    const modal = screen.getByText('Bem-vindo à Caverna da Gordura').closest('div');
