...
export const AdminProvider = ({ children }: { children: React.ReactNode }) => {
  ...
  const adicionarAvaliacao = useCallback(
    (produtoId: string, review: Omit<Review, "id">) => {
  ...
  const value = useMemo(
    () => ({ produtos, alternarDisponibilidade, adicionarAvaliacao }),
    [produtos, alternarDisponibilidade, adicionarAvaliacao],
  );

  return (
    <AdminContexto.Provider value={value}>{children}</AdminContexto.Provider>
  );
};
